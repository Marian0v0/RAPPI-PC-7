...
const DashboardRestaurante = () => {
  ...
  useEffect(() => {
    ...
    const restauranteData = localStorage.getItem('restaurante');
    if (restauranteData) {
      setRestaurante(JSON.parse(restauranteData));
      loadData(JSON.parse(restauranteData));
    } else {
  ...
  const loadData = async (restauranteData) => {
    try {
      setLoading(true);
      
      // Cargar productos del restaurante
      const productosResponse = await fetch(`https://rappi-pc-7.onrender.com/backend/productos/restaurante/${restauranteData.id_restaurante}`);
      ...
      // Cargar pedidos del restaurante (mock por ahora)
      const pedidosMock = [
        {
          id_pedido: 1,
          id_cliente: 1,
          ...
          productos: [
            ...
            { nombre: 'Papas', cantidad: 1, precio: 10000 }
          ]
        },
        {
          id_pedido: 2,
          ...
          total_pedido: 18000,
          productos: [
            { nombre: 'Pizza', cantidad: 1, precio: 18000 }
          ]
        }
      ];
  ...
  const handleEditProduct = async (producto) => {
    try {
      const response = await fetch(`https://rappi-pc-7.onrender.com/backend/productos/${producto.id_producto}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          detalles_producto: producto.detalles_producto,
          precio_producto: producto.precio_producto,
          fotografia_producto: producto.fotografia_producto
        }),
      });
  ...
  const updatePedidoEstado = async (pedidoId, nuevoEstado) => {
    try {
      // Aquí se implementaría la actualización del estado del pedido
    ...
    <div className="dashboard-container">
      ...
      <div className="dashboard-sidebar">
        <div className="sidebar-header">
          <h2>Dashboard</h2>
          <p>{restaurante?.nombre_restaurante}</p>
        </div>
      ...
      <div className="dashboard-main">
        <div className="dashboard-header">
          ...
          <div className="header-actions">
            <span className="welcome-text">Bienvenido, {restaurante?.nombre_restaurante}</span>
          </div>
        </div>
            ...
            <div className="perfil-tab">
              ...
              <div className="perfil-info">
                ...
                <div className="info-group">
                  ...
                  <p>{restaurante?.direccion_restaurante}</p>
                </div>
                <div className="info-group">
                  <label>Teléfono</label>
                  <p>{restaurante?.cel_restaurante}</p>
                </div>
                <div className="info-group">
                  <label>Email</label>
                  <p>{restaurante?.email_restaurante}</p>
                </div>
              </div>
