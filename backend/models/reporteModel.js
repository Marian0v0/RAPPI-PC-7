...
const Reporte = {
    ...
    getClientReports: async(id_cliente) =>{
        const { data, error } = await supabase.from('reportes').select().eq('id_cliente', id_cliente);
    ...
    getByType: async(tipo_reporte) =>{
        const { data, error } = await supabase.from('reportes').select().eq('tipo_reporte', tipo_reporte);
    ...
    delete: async (id_reporte) => {
        const { error } = await supabase.from('reportes').delete().eq('id_reporte', id_reporte);
